...
namespace handsome{
    ...
    {
        LOG_DEBUG << "tcp_connection::dtor[" << m_name << "] at " << this << " fd = " << m_socket->fd();
    }
    ...
    void tcp_connection::send_in_loop(const std::string &message)
    {
        ...
        if(!m_channel->is_writing() && m_output_buffer.readable_bytes() == 0){
            num_wrote = ::write(m_socket->fd(), message.data(), message.size());
            if(num_wrote >= 0){
                if(implicit_cast<size_t>(num_wrote) < message.size()){
                    LOG_TRACE << "I am going to write more data";
                }else if(m_write_complete_cb){
                    m_loop->queue_in_loop(std::bind(m_write_complete_cb, shared_from_this()));
                }
            }else{
                num_wrote = 0;
    ...
    void tcp_connection::shutdown()
    {
        if(m_state == kConnected){
            ...
            m_loop->run_in_loop(std::bind(&tcp_connection::shutdown_in_loop, shared_from_this()));
        }
    }
    ...
    {
        int save_errno = 0;
        ssize_t n = m_input_buffer.read_fd(m_socket->fd(), &save_errno);
        if(n > 0){
    ...
    void tcp_connection::handle_write()
    {
        m_loop->assert_in_loop_thread();
        if(m_channel->is_writing()){
            ::ssize_t n = ::write(m_socket->fd(), m_output_buffer.peek(), m_output_buffer.readable_bytes());
            ...
            if(n > 0){
                ...
                if(m_output_buffer.readable_bytes() == 0){
                    ...
                    if(m_write_complete_cb){
                        m_loop->run_in_loop(std::bind(m_write_complete_cb, shared_from_this()));
                    }
    ...
    {
        int err = socket_ops::get_socket_error(m_socket->fd());
