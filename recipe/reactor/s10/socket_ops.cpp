...
namespace handsome{
        ...
        if(connfd < 0){
            ...
            switch(saved_errno){
                case EAGAIN:
                    ...
                    break;
                case EBADF:
                    LOG_SYSFATAL << "socket_ops::accept unexpected error: " << saved_errno;
                    break;
            }
        }
    ...
    int socket_ops::get_socket_error(int sockfd)
    {
        ...
        int ret = ::getsockopt(sockfd, SOL_SOCKET, SO_ERROR, &opt_val, &opt_len);
        if(ret < 0){
            LOG_SYSERR << "socket_ops::get_socket_error";
            return errno;
        }else{
            return opt_val;
        }
