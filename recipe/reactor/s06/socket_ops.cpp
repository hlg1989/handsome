...
namespace handsome{
    ...
    int socket_ops::create_non_blockint_or_die()
    {
        int sockfd = ::socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
        if(sockfd < 0){
            LOG_SYSFATAL << "socket_ops::create_non_blockint_or_die()";
        }
    ...
    int socket_ops::accept(int sockfd, struct sockaddr_in *peer_addr)
    {
        ...
        int connfd = ::accept(sockfd, sockaddr_cast(peer_addr), &addr_len);
    ...
    struct sockaddr_in socket_ops::get_local_addr(int sockfd)
    {
