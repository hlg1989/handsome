...
namespace handsome{
    ...
    timestamp poller::poll(int timeout_ms, channel_list_t *active_channels)
    {
        ...
        if(num_events > 0){
            ...
            fill_active_chennels(num_events, active_channels);
    ...
    void poller::fill_active_chennels(int num_events, channel_list_t *active_channels) const
    {
    ...
    void poller::remove_channel(channel *ch)
    {
        ...
        if(implicit_cast<size_t>(index) == m_pollfds.size() - 1){
            ...
            std::iter_swap(m_pollfds.begin() + index, m_pollfds.end());
            if(channel_at_end < 0){
